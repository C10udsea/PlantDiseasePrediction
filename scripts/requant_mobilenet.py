"""Re-quantize MobileNetV2 to static INT8 using saved calibration data."""
import json
import time
...
def main():
    ...
    v.update({"size_mb": OUT.stat().st_size / 1e6, "acc": q_acc, "macro_f1": q_f1,
              "n": int(len(ev_y)), "note": "1024 calib / 512 eval, per_channel"})
    so = sess_q.get_session_options()
    so.intra_op_num_threads = 1
    dummy = np.asarray(ev_x[:1])
    for _ in range(20):
        sess_q.run(["logits"], {"input": dummy})
    ts = []
    for _ in range(100):
        t = time.perf_counter()
        sess_q.run(["logits"], {"input": dummy})
        ts.append((time.perf_counter() - t) * 1000)
    v["cpu_ms_median_t1"] = float(np.median(ts))
    v["cpu_ms_p90_t1"] = float(np.percentile(ts, 90))
